...
const namespace = 'component.container.properties';
export class Properties {
    ...
    */
    constructor(Id) {
        ...
        Object.freeze(this);
        const ref = new Reference(namespace, Properties, Id);
        if (privateBag.has(ref)) {
            ...
            for (const Class of extendedClasses) {
                ...
                for (const propertyKey of classProperties) {
                    const propertyId = new GUID({ Id: Id.toString(), propertyKey })
                    const { get, set } = Reflect.getOwnPropertyDescriptor(Class.prototype, propertyKey);
                    if (get) {
                        properties.push(new Property(propertyId, get, null, propertyKey, targetClass));
                    }
                    if (set) {
                        properties.push(new Property(propertyId, null, set, propertyKey, targetClass));
                    }
                }
            }
