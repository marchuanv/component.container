...
export class Container extends TypeReference {
    constructor() {
        ...
        let { ctorArgsClass, Class } = config.find(targetClass);
        if (ctorArgsClass === null || ctorArgsClass === undefined) {
            ({ ctorArgsClass, Class } = config.find(null, targetClass));
        }
        if (ctorArgsClass === null || ctorArgsClass === undefined || Class === undefined || Class === null) {
            throw new Error('container configuration error.');
        }
        if (targetClass === ctorArgsClass) {
            try {
                const { refId } = containerStack.get(ctorArgsClass);
                super(new TypeReferenceContext(new TypeRegisterEntry(targetClass), false, refId));
                containerStack.set(Class, new ReferenceId());
            } catch(error) {
                throw new Error('container was waiting for ctorArgs');
            }
        } else if (targetClass === ctorArgsClass) {
            try {
                const { refId } = containerStack.get(Class);
                super(new TypeReferenceContext(new TypeRegisterEntry(Class), false, refId));
            } catch(error) {
                throw new Error('container was waiting for ctorArgs');
            }
        } else {
