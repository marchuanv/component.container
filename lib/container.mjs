...
class ContainerTypeRegister extends TypeRegister {}
class ContaineTypeReference extends TypeReference {
    constructor(typeRegId, refId) {
        super(new TypeReferenceContext(new ContainerTypeRegister(typeRegId), false, refId));
    }
}
export class Container extends TypeReference {
    constructor() {
        ...
        let { refId, ctorArgsClass } = config.get(targetClass);
        if (ctorArgsClass !== null) {
            const referenceStack = new ReferenceStack();
            let { typeRegId } = referenceStack.get(ctorArgsClass);
            const instance = new ContaineTypeReference(typeRegId, refId);
            console.log();
        }
