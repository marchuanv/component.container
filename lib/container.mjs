...
export class Container extends TypeReference {
    constructor() {
        ...
        let ctorArgsTopOfStack = false;
        if (ctorArgsClass !== null) {
            const referenceStack = new ReferenceStack();
            let topRegister = referenceStack.get();
            let started = topRegister;
            while (topRegister) {
                let { type } = topRegister.get();
                if (ctorArgsClass === type) {
                    ctorArgsTopOfStack = true;
                    break;
                }
                topRegister = referenceStack.get();
                if (topRegister === started) {
                    break;
                }
            }
        }
