...
export class Container extends Properties {
    ...
    */
    static register(configClass) {
        const config = new configClass();
        const references = config.namespaces
            .filter(ns => Reference.has(ns, Config, config.Id))
            .map(ns => Reference.get(ns, Config, config.Id));
        const configurations = references.map(ref => ref.get());
        console.log();

        // if (!Type.has('component', targetClass.name)) {
        //     new Type('component', targetClass);
        // }
        // if (!Type.has('component', ctorArgsClass.name)) {
        //     new Type('component', ctorArgsClass);
        // }
    }
    /**
