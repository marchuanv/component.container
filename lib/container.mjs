...
export class Container extends TypeReference {
    ...
    */
    constructor(targetClass, refId) {
        ...
        if (target !== ContainerReference.prototype && target !== ContainerReference) {
    ...
    */
    static create(targetClass, contract) {
        ...
        const container = new ContainerReference(targetClass, targetClassRefId);
        const _contract = new ContainerReference(contract.constructor, interfaceRefId);
        for(const prop of container.properties){
            if (!_contract.properties.find(x => x.name === prop.name)) {
...
class ContainerReference extends Container {
    constructor(targetClass, targetClassRefId) {
        super(targetClass, targetClassRefId);
    }
}